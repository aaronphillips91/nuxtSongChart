<template>
  <div class="flex flex-col gap-4 px-4 my-4 7xl:mx-auto scPage">
    <div class="flex flex-col gap-2">
      <h4>Profile Setup</h4>
      <p class="text-xs">
        Welcome to SongChart! Let's get your profile set up so you can start
        exploring and sharing your musical journey. Please add your name and a
        profile picture to personalize your experience. If you'd like, you can
        also provide a contact phone number for seamless support and updates.
        Finally, choose a subscription tier that best suits your needs and
        unlock the full potential of SongChart. We're excited to have you on
        board and can't wait to see where your music takes you!
      </p>
    </div>
    <UDivider />
    <div>
    ...
    <div class="flex gap-4">
      <div
        ...
        <div class="mb-4">
          <div v-for="n in tier.points" :key="n" class="mb-2 text-xs">
            - {{ n }}
          </div>
        </div>
        <UButton @click="setTier(tier.name)" block class="mt-auto"
          >Select {{ tier.name }}</UButton
        >
      </div>
    </div>
