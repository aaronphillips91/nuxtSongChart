<template>
  <UModal v-model="modal">
    <div class="flex flex-col gap-4 p-4">
      ...
      <UDivider/>
      <BasePicUploader @file-selected="handleFile" label="Album Art" intent="album"/>
      <BaseInput label="Song title" v-model="song.title.value"/>
      <div class="flex w-full gap-4">
        <BaseInput class="w-full" label="Album name" v-model="song.album.value"/>
        <BaseInput class="w-full" label="Artist name" v-model="song.artist.value"/>
      </div>
      <div class="flex w-full gap-4">
        <BaseInput class="w-full" label="Tempo" v-model="song.tempo.value"/>
        <BaseInput class="w-full" label="Original key" v-model="song.original_key.value"/>
      </div>
